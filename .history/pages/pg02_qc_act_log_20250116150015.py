...
# 検査項目のマスターデータの読み込み
try:
    Measurement_Item = pd.read_excel('master_folder/measurement_item.xlsx')
except ImportError:
...
with st.sidebar:
    ...
    try:
        ...
        # フルネームのリストを作成
        if 'Full_Name' in employee_data.columns:
            implementor_options = employee_data['Full_Name'].tolist()
        else:
            st.error("フルネームの列が見つかりません。")
            st.stop()
    except ImportError:
        st.error(
            "openpyxlがインストールされていません。pipまたはcondaを使用してopenpyxlをインストールしてください。"
        )
        st.stop()
