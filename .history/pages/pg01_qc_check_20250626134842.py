...
class QCCheckApp:
    ...
    async def display_data_summary(self, data: ProcessedData) -> None:
        """データ概要の表示（最適化版）"""
        metrics = {
            "ファイル情報": len(data.file_info) if isinstance(data.file_info, pd.DataFrame) else 0,
            "ICデータ": len(data.ic_data) if isinstance(data.ic_data, pd.DataFrame) else 0,
            "NCデータ": (
    ...
    async def run(self) -> None:
        ...
        async with self.error_handler():
                ...
                with tab2:
                    ...
                    if not file_list.empty:
                        ...
                        if file_options:
                            ...
                            if selected_file_name and st.button(
                                "選択したファイルのデータを表示"
                            ):
                                # 選択されたファイルのレコードを取得
                                file_records = await self.get_file_records(
                                    selected_file_name
                                )
                                ...
                                if not file_records["file_info"].empty:
                                    ...
                                    st.success(
                                        f"ファイル '{selected_file_name}' のデータを読み込みました"
                                    )
                                else:
                                    st.warning(
                                        "選択されたファイルのデータが見つかりませんでした"
                                    )
                    else:
            ...
            # 初期状態のメッセージ
                ...
                st.info(
                    "サイドバーからファイルをアップロードするか、データベースからファイルを選択してください"
                )
