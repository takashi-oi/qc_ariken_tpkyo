...
QUERY = """
    SELECT File_Name, Date_Time, Batch
...
with col1:
    ...
    standard_usage = st.radio(
        '標準物質の使用状況を選択してください',
        ('使用なし', '継続使用', '開封 or 調整 /Lot変更なし', '開封 or 調整 /Lot変更あり')
    )

    st.markdown("#### 管理試料の使用状況")
    control_usage = st.radio(
        '管理試料の使用状況を選択してください',
        ('継続使用', '開封 or 調整 /Lot変更なし', '開封 or 調整 /Lot変更あり')
    )
...
with col2:
    ...
    reagents_usage = st.radio(
        '検査試薬の使用状況を選択してください',
        ('継続使用', '開封 or 調整 /Lot変更なし', '開封 or 調整  /調整/Lot変更あり')
    )
...
if st.button("データの確認 / 登録"):
    df_check_list = pd.DataFrame({
        ...
        'ファイル名': [st.session_state.get('file_name', '')],
    ...
    try:
        ...
        db_cursor.execute("""
            ...
            WHERE Date_Time = ? AND Batch = ? AND File_Name = ?
        """, (
