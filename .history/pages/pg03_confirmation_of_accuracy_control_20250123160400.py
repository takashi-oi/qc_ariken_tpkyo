    ...
    def render_sidebar() -> Tuple[date, str, str]:
        ...
        with st.sidebar:
            try:
                ...
                selected_date = selected_date[0] if isinstance(selected_date, tuple) else selected_date
                return selected_date, measurement, measurer
            except (AttributeError, TypeError) as e:
                logger.warning("日付の取得に失敗: %s", e)
                measurement = "Precision Manager"
                measurer = ""
                return date.today(), measurement, measurer
