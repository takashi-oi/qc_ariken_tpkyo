...
try:
    ITEM_CODE_SELECT = pd.read_excel('master_folder/measurement_item.xlsx')  # Excelファイルから検査項目を読み込む
    # 列名のリストを取得
    ...
    for target, possibilities in column_pairs:
        found = next((col for col in possibilities if col in columns), None)  # 実際の列名を見つける
        if not found:
            st.error(
    ...
    st.error("measurement_item.xlsxファイルが見つかりません")  # ファイルが見つからない場合のエラーメッセージ
    st.stop()  # 処理を停止
except pd.errors.EmptyDataError:
    st.error("measurement_item.xlsxファイルが空です")  # ファイルが空の場合のエラーメッセージ
    st.stop()  # 処理を停止
except pd.errors.ParserError:
    st.error("measurement_item.xlsxファイルの解析に失敗しました")  # 解析エラーの場合のエラーメッセージ
    st.stop()  # 処理を停止
except PermissionError:
    st.error("measurement_item.xlsxファイルにアクセスする権限がありません")  # アクセス権限エラーの場合のエラーメッセージ
    st.stop()  # 処理を停止
...
with st.sidebar:
    ...
    # 実施者名の入力
    try:
        employee_data = pd.read_excel("master_folder/employee_code.xlsx")  # 実施者名のデータを読み込む
        measurer = st.selectbox(
            "測定者名",  # セレクトボックスのラベル
            employee_data["Member's_Name"].tolist()  # 実施者名のリストを表示
        )
    except FileNotFoundError:
        st.error("master_folder/employee_code.xlsxファイルが見つかりません")  # ファイルが見つからない場合のエラーメッセージ
        st.stop()  # 処理を停止
    except pd.errors.EmptyDataError:
        st.error("employee_code.xlsxファイルが空です")  # ファイルが空の場合のエラーメッセージ
        st.stop()  # 処理を停止
    except pd.errors.ParserError:
        st.error("employee_code.xlsxファイルの解析に失敗しました")  # 解析エラーの場合のエラーメッセージ
        st.stop()  # 処理を停止
    except PermissionError:
