        ...
        for title, df in data_sections.items():
            with st.expander(title, expanded=True):
                if isinstance(df, pd.DataFrame) and not df.empty:
                    ...
                    st.write(f"**データ列:** {list(df.columns)}")
                    st.dataframe(df, use_container_width=True)

                    # データの基本統計情報を表示
                    if "Ct" in df.columns:
                        st.write("**Ct値の統計:**")
                        st.write(df["Ct"].describe())
                else:
                    st.warning(f"⚠️ {title}のデータがありません")
                    if isinstance(df, pd.DataFrame):
                        st.info(f"空のDataFrame（{len(df)}行）")
                    else:
                        st.info("データがNoneまたはDataFrameではありません")
            ...
            if uploaded_file and measurer:
                try:
                    ...
                    if (
                        ...
                        st.error(
                            "❌ データの読み込みに失敗しました。ファイル形式を確認してください。"
                        )

                except Exception as e:
                    st.error(f"❌ データ処理中にエラーが発生しました: {str(e)}")
                    st.info(
                        "💡 ファイル名の形式が正しいか確認してください（例：CODE_BATCH-YYYYMMDD.xlsx）"
                    )
                ...
                st.info(
                    "📋 QCデータファイルをアップロードし、測定者名を選択してください"
                )
