        ...
        if selected_file_name:
            try:
                ...
                if not multi_rule_df.empty:
                    ...
                    # タイプのユニーク値リストを取得し、セレクトボックスを設置
                    if "Type" in filtered_multi_rule_df.columns:
                        type_options = (
                            filtered_multi_rule_df["Type"].dropna().unique().tolist()
                        )
                    else:
                        type_options = []
                    type_label = "タイプを選択"
                    type_select = st.selectbox(
                        type_label, options=["すべて"] + type_options
                    )

                    # タイプでフィルタ
                    if type_select != "すべて":
                        filtered_multi_rule_df = filtered_multi_rule_df[
                            filtered_multi_rule_df["Type"] == type_select
                        ]
