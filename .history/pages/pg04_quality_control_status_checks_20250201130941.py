...
class DatabaseManager:
    ...
    def __init__(self, config: DatabaseConfig = DatabaseConfig()):
        ...
        if not os.path.exists(self.config.path):
            logger.error(f"データベースファイルが見つかりません: {self.config.path}")
            raise FileNotFoundError(f"データベースファイルが見つかりません: {self.config.path}")
    ...
    def get_connection(self) -> Generator[sqlite3.Connection, None, None]:
        ...
        for attempt in range(self.config.max_attempts):
            try:
                ...
                logger.error(f"データベース接続エラー (試行 {attempt + 1}/{self.config.max_attempts}): {e}")
                if attempt == self.config.max_attempts - 1:
                    st.error("データベースに接続できません。システム管理者に連絡してください。")
                    raise
            finally:
        if conn is not None:
            try:
    ...
    def fetch_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        if not start_date or not end_date:
            logger.error("開始日または終了日が指定されていません")
            st.error("日付を正しく指定してください")
            return pd.DataFrame()
        ...
        try:
            with self.get_connection() as conn:
                ...
                if df.empty:
                    logger.warning(f"指定期間 {start_date} から {end_date} のデータが見つかりません")
                    st.warning("指定された期間のデータが見つかりません")
                return df
        except (sqlite3.Error, pd.errors.DatabaseError) as e:
            logger.error(f"データ取得エラー: {e}")
            st.error("データの取得に失敗しました。システム管理者に連絡してください。")
            return pd.DataFrame()
