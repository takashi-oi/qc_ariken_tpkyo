        ...
        if processed_data:
            ...
            with st.expander("ICデータの詳細", expanded=True):
                if not processed_data.ic_data.empty:
                    st.dataframe(processed_data.ic_data,
                                 use_container_width=True)
                else:
                    st.info("ICデータはありません")
            ...
            with st.expander("NCデータの詳細", expanded=True):
                if not processed_data.nc_data.empty:
                    st.dataframe(processed_data.nc_data,
                                 use_container_width=True)
                else:
                    st.info("NCデータはありません")

            with st.expander("PCデータの詳細", expanded=True):
                if not processed_data.pc_data.empty:
                    st.dataframe(processed_data.pc_data,
                                 use_container_width=True)
                else:
                    st.info("PCデータはありません")
