...
def main():
    ...
    # 最近の登録表示
    st.subheader("最近の登録")
    recent_records = data_access.get_recent_records(Config.recent_records_limit)
    ...
    # サイドバーで「表示」ボタンが押された場合のみ、ファイル名でtable_qc_status_logからデータ抽出
    if show_status_log:
        ...
        if selected_file_name:
            try:
                ...
                if not file_info_df.empty:
                    st.subheader("測定情報")
                    # display_file_infoをfile_info_dfのコピーとして定義
                    display_file_info = file_info_df.copy()

                    # 必要な5カラムのみ抽出（ファイル名を除外）
                    if len(display_file_info.columns) >= 6:
                        # 2列目から6列目を抽出（ファイル名を除外）
                        ...
                        f"予期しない列数です: {len(display_file_info.columns)}列"
                ...
                try:
                    ...
                    if not multi_rule_df.empty:
                        ...
                        st.info("該当ファイルのQCマルチルール情報はありません。")
                except (sqlite3.Error, pd.errors.DatabaseError, FileNotFoundError) as e:
                    st.error(f"QCマルチルール情報の取得に失敗しました: {e}")
