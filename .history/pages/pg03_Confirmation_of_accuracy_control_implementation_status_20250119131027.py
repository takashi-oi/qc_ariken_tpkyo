...
class DatabaseManager:
    ...
    def __init__(self, db_path):
        ...
        # 必須フィールドの検証
        if check_log_df is None or not isinstance(check_log_df, pd.DataFrame):
            st.error("チェックログデータフレームが無効です。")
            return False

        required_fields = ['date_time', 'type', 'SD_conversion']
        for field in required_fields:
            if field not in check_log_df.columns:
                st.error(f"必須フィールド '{field}' が見つかりません。")
                return False
        check_log_df.to_sql('table_qc_act_log', conn, if_exists='append', index=False)
        st.success("チェックログが正常に保存されました。")
        return True
        ...
        try:
            ...
            if conn is not None:
                return pd.read_sql_query(
                    query.replace('Date_Time', 'date_time'),
                    conn)
            return pd.DataFrame()
