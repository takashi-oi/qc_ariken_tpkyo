...
# アプリケーション設定
st.set_page_config(page_title="検査測定状況管理記録登録", layout="wide")
...
def main():
            ...
            try:
                ...
                if not multi_rule_df.empty:
                    ...
                    for col in columns_to_show:
                        if col in multi_rule_df.columns:
                            available_cols.append(col)
                    display_multi_rule = multi_rule_df[available_cols].copy()
                    jp_columns = [
                        "項目コード",
                        "バッチ",
                        "機種",
    ...
    # QCデータ登録フォーム（最下部に移動）
    # ファイル情報があれば、項目コード・日時・バッチを取得
    file_info_item_codes = []
    file_info_dates = []
    file_info_batches = []
    if (
        "file_info_df" in locals()
        and file_info_df is not None
        and not file_info_df.empty
        and all(col in file_info_df.columns for col in ["項目コード", "日時", "バッチ"])
    ):
        file_info_item_codes = file_info_df["項目コード"].dropna().unique().tolist()
        file_info_dates = file_info_df["日時"].dropna().unique().tolist()
        file_info_batches = file_info_df["バッチ"].dropna().unique().tolist()

    with st.form("qc_data_form"):
        st.subheader("QCデータ登録")

        # 項目選択
        if file_info_item_codes:
            item_options = file_info_item_codes
        elif not measurement_df[0].empty:
            item_options = [
                f"{code} - {name}"
                for code, name in zip(
                    measurement_df[0]["Item_Code"],
        ...
        # 選択した検査項目に紐づくファイル情報レコードを表示
        if file_info_item_codes and selected_item_code:
            filtered_info = file_info_df[
                file_info_df["項目コード"] == selected_item_code
            ]
            if not filtered_info.empty:
                st.markdown(
                    f"#### 選択した検査項目のファイル情報（{selected_item_code}）"
                )
                st.dataframe(filtered_info)
            else:
                st.info("該当するファイル情報がありません。")

        # 日付時間の入力
        if file_info_dates:
            date_time = st.selectbox("実施日", options=file_info_dates)
        else:
            date_time = st.date_input("実施日")

        # バッチの入力
        if file_info_batches:
            batch = st.selectbox("バッチ", options=file_info_batches)
        else:
        ...
        # 違反ルール選択
        violated_rule_df = data_access.get_violated_rules(selected_item_code)
        if not violated_rule_df.empty and "Violated_rule" in violated_rule_df.columns:
            violated_rule_options = violated_rule_df["Violated_rule"].tolist()
        else:
            violated_rule_options = []
        violated_rule = st.selectbox("違反ルール", options=[""] + violated_rule_options)
