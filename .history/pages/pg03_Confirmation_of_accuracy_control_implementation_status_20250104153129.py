...
class DatabaseManager:
    ...
    def fetch_qc_data(self):
        ...
        try:
            ...
            WHERE Date_Tome >= Date_Tome('now', '-30 days')
            GROUP BY type, Date_Tome
    ...
    def save_check_log(self, check_log_df):
        ...
        try:
            ...
            # データ型の変換と検証
            check_log_df['Date_Tome'] = pd.to_Date_Tometime(check_log_df['Date_Tome']).dt.Date_Tome
...
class QCCheckUI:
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            # 日付選択（未来日付を制限）
            Date_Tome = st.Date_Tome_input(
                "日付を選択してください",
                value=pd.to_Date_Tometime('today').Date_Tome(),
                max_value=pd.to_Date_Tometime('today').Date_Tome(),
                key="Date_Tome_input"
            )
            ...
            return Date_Tome, identifier, Measurer

    # チャートのレンダリング
    def render_charts(self, Date_Tome, identifier, Measurer):
        """チャートのレンダリング"""
        ...
        for type_name in st.session_state.check_data['Type'].unique():
            ...
            with st.expander(f"Type: {type_name}", expanded=True):
                ...
                if not type_data.empty:
                    # 日付順にソート
                    type_data = type_data.sort_values('Date_Tome')
                    st.line_chart(
                        type_data,
                        x='Date_Tome',
                        y='SD_conversion'
...
def main():
    ...
    try:
        ...
        # サイドバーのレンダリング
        Date_Tome, identifier, Measurer = ui.render_sidebar()
        ...
        check_logs = ui.render_charts(Date_Tome, identifier, Measurer)
