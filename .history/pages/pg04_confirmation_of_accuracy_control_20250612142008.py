...
class QCCheckUI:
    ...
    def render_charts(self) -> List[Dict[str, str]]:
        """チャートを描画し、QCログを生成"""
        check_data = st.session_state.get('check_data',
                                          (pd.DataFrame(),
                                           pd.DataFrame()))
        ...
        if qc_data is None or (
              isinstance(qc_data, pd.DataFrame) and qc_data.empty):
        ...
        try:
            ...
            for type_name, group in qc_data.groupby('Type'):
                with st.expander(f"Type: {type_name}", expanded=True):
                    st.line_chart(
                        group.set_index('date_time')['sd_conversion']
                    )

                    # ステータスデータを表示
                    status_group = status_data[
                        status_data['status_type'] == type_name]
