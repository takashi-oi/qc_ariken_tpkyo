...
def main():
    ...
    selected_recent_file = st.sidebar.selectbox(
        "直近で読み込んだファイル名",
        options=recent_files if recent_files else ["履歴なし"],
        key="sidebar_recent_file"
    )
    # 今後の機能拡張で利用予定（未使用変数警告抑制）
    _ = selected_recent_file
    ...
    with tab1:
        ...
        with st.form("qc_data_form"):
            ...
            # 実施者選択
            employee_code_df = pd.read_excel(
                "master_folder/employee_code.xlsx")
            # サイドバーで選択した担当者をデフォルト値に設定
            implementor_options = [""] + employee_code_df[
                "Member's_Name"
            ].tolist()
            member_names = employee_code_df["Member's_Name"].tolist()
            if sidebar_implementor in member_names:
                implementor_index = member_names.index(sidebar_implementor) + 1
            else:
                implementor_index = 0
            implementor = st.selectbox(
                "実施者",
                options=implementor_options,
                key="implementor",
                index=implementor_index
            )
