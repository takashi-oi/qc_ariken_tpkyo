...
class QCCheckUI:
    ...
    def initialize_session_state(self):
        ...
        if self.input_values is None:
            with st.sidebar:
                ...
                self.input_values = (Date_Time, measurement, measurer)
    ...
    def render_charts(self, Date_Time, measurement, measurer):
        ...
        # データフレームの列名を確認
        st.write("デバッグ: 利用可能な列", st.session_state.check_data.columns.tolist())
        ...
        for type_name in st.session_state.check_data['type'].unique():
            with st.expander(f"Type: {type_name}", expanded=True):
                ...
                if not type_data.empty:
                    # 列名が存在することを確認してからソート
                    if 'date_time' in type_data.columns:
                        type_data = type_data.sort_values('date_time')
                        st.line_chart(
                            type_data,
                            x='date_time',
                            y='sd_conversion'  # 大文字に変更
                        )
                    else:
                        st.error("date_time列が見つかりません")
                        st.write("利用可能な列:", type_data.columns.tolist())
...
def main():
    ...
    try:
        ...
        # initialize_session_stateメソッドから日付、測定者、責任者を取得
        Date_Time, measurement, measurer = ui.initialize_session_state()
