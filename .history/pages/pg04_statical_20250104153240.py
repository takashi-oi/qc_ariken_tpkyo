...
def load_qc_data(implementation_Date_Tome, conn):
    """データベースからQCデータを読み込んで処理する"""
    # SQLクエリを作成して必要なカラムを指定
    qc_data_query = f"""
    SELECT `Date_Tome`, `Batch`, `item_code`, `Model`, `type`, `Lot Number`,
           `sd_conversion`, `Ct`, `judgment`, `Error_type`, `Violated_rule`
    FROM table_qc_mulch_rule
    WHERE `Date_Tome` <= '{implementation_Date_Tome}'  # 指定された日付以前のデータのみを取得
    ORDER BY `Date_Tome` DESC  # 日付の降順でソート
    """
    # SQLクエリを実行してデータフレームに変換
    df_qc_query = pd.read_sql_query(qc_data_query, conn)
    # 日付とバッチで昇順にソートして返す
    return df_qc_query.sort_values(['Date_Tome', 'Batch'], ascending=True)
...
# キャッシュ機能を使用して高速化
@st.cache_data
def create_excel_report(dfs, year, month):
    """QCデータのExcelレポートを作成"""
...
def main():
    ...
    if year and month:
        ...
        implementation_Date_Tome = f"{year}-{month.zfill(2)}-{last_day}"
        ...
        with sqlite3.connect('db_folder/qc_data_base.db', timeout=30) as conn:
            ...
            # QCデータを読み込み、基本統計量を計算
            df_qc_data = load_qc_data(implementation_Date_Tome, conn)
