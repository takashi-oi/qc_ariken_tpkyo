...
def main():
    ...
    with tab1:
        ...
        with st.form("qc_data_form"):
            ...
            # 実施者選択
            if not employee_df.empty and "実施者名" in employee_df.columns:
                implementor = st.selectbox("実施者", options=[""] + employee_df["実施者名"].tolist())
            else:   
                implementor = st.selectbox("実施者", options=[""])
