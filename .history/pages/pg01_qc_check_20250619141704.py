    ...
    async def handle_database_operations(self, data: ProcessedData) -> None:
        ...
        if st.button("データベース登録／マルチルールチェック開始"):
            ...
            async with self.error_handler():
                ...
                # --- ここから追加: マルチルール判定結果をqc_multi_ruleに登録 ---
                # PCデータのマルチルール判定結果を取得
                pc_df = data.pc_data if isinstance(data.pc_data, pd.DataFrame) else None
                    ...
                    async with get_db_connection(Config.DATABASE_PATH) as conn:
                        try:
                            ...
                            if not historical_data.empty:
                                for type_name, group in historical_data.groupby("Type", as_index=False):
                                    ...
                                    for _, row in rule_results.iterrows():
                                        insert_row = {
                                            ...
                                            "type": row.get("Type", ""),
                                            "QC_RESULTS": row.get("Judgment", ""),
                                            "Violated_rule": row.get("Violated_rule", "")
                                        }
                                        # DataFrame化してto_sqlで登録
                                        insert_df = pd.DataFrame([insert_row])
                                        insert_df.to_sql("qc_multi_rule", conn, if_exists="append", index=False)
                        except Exception as e:
                            st.error(f"qc_multi_rule登録時エラー: {e}")
                # --- 追加ここまで ---
    ...
    async def run(self) -> None:
        ...
        async with self.error_handler():
            ...
            if uploaded_file and measurer:
                ...
                if qc_data and isinstance(qc_data.file_info,
                                          pd.DataFrame
