...
with st.sidebar:
    ...
    if item_code:
        ...
        cursor.execute("""
            ...
            LIMIT 5
        """, (item_code,))
        date_time_list = [row[0] for row in cursor.fetchall()]
        
        if date_time_list:
            date_time = st.selectbox('実施日を選択してください', date_time_list)
            ...
            if date_time:
                ...
                cursor.execute("""
                    ...
                    FROM qc_multi_rule
                    WHERE Date_Time = ? AND Item_Code = ?
                    ORDER BY Batch
                """, (date_time, item_code))
                batch_list = [row[0] for row in cursor.fetchall()]
                
                if batch_list:
                    batch = st.selectbox('測定「Batch」を選択してください', batch_list)
                    ...
                    if batch:
                        ...
                        type_list = [row[0] for row in cursor.fetchall()]
                        
                        if type_list:
                            type_variable = st.selectbox('検査項目のTypeを選択してください', type_list)
                            ...
                            if type_variable:
                                ...
                                rule_list = [row[0] for row in cursor.fetchall()]
                                
                                if rule_list:
                                    violated_rule = st.selectbox('Multi Rule判定結果を選択してください', rule_list)
