...
try:
    ...
    column_pairs = [
        ...
        ('Measurement Item', ['Measurement Item', 'MeasurementItem', '測定項目', '項目名'])
    ]
    ...
    for target, possibilities in column_pairs:
        ...
        if not found:
            st.error(f"必要な列 '{target}' が見つかりません。利用可能な列: {', '.join(columns)}")
            st.stop()
    ...
    st.error(f"検査項目マスターデータの読み込みエラー: {str(e)}")
    st.stop()
...
with st.sidebar:
    ...
    try:
        ...
        # 列名のリストを取得
        columns = employee_data.columns.tolist()

        # 想定される列名のリスト
        possible_name_columns = ['氏名', '名前', 'フルネーム', '従業員名', 'Full_Name', 'Name']

        # 存在する列名を探す
        name_column = next((col for col in possible_name_columns if col in columns), None)

        if name_column:
            implementor_options = employee_data[name_column].tolist()
        else:
            st.error(f"従業員名の列が見つかりません。利用可能な列: {', '.join(columns)}")
            implementor_options = ['データ読み込みエラー']

    except FileNotFoundError:
        st.error("employee_code.xlsxファイルが見つかりません")
        implementor_options = ['ファイルが見つかりません']
    except Exception as e:
        st.error(f"従業員データの読み込みエラー: {str(e)}")
        implementor_options = ['データ読み込みエラー']

    implementor = st.selectbox('実施者名を選択してください', implementor_options)
