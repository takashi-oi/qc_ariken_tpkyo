...
class QCCheckApp:
    ...
    async def run(self) -> None:
        """アプリケーションのメイン実行関数"""
        async with self.error_handler():
            uploaded_file = st.file_uploader("QCデータファイルをアップロード", type=["xlsx"])
            measurer = st.text_input("測定者名を入力")
            if uploaded_file and measurer:
                qc_data = await self.data_importer.process_and_display(uploaded_file, measurer)
