...
with col1:
    ...
    standard_usage = st.radio(
        ...
        '標準物質の使用状況を選択してください',
        # 選択肢
        ('該当なし / 使用なし', '開封済み / 継続使用', '開封 / Lot変更なし', '開封 / Lot変更あり')
    )
...
with st.sidebar:  # サイドバーにウィジェットを配置
    ...
    try:  # 実施者名の入力と保存を試みる
        ...
        selected_measurer = st.selectbox(  # ドロップダウンリスト
            "測定者名", employee_data["Member's_Name"].tolist())  # ラベルと選択肢
        # 選択された測定者をセッション状態に保存
        st.session_state['implementor'] = selected_measurer
    except FileNotFoundError:  # ファイルが見つからない場合
        st.error(  # エラーメッセージ
            "data/master/employee_code.xlsxファイルが"
            "見つかりません"
        )
        st.stop()  # プログラムを停止
    except (pd.errors.EmptyDataError,  # データの読み込みに失敗した場合
...
if st.button("データの確認 / 登録"):  # データの確認 / 登録ボタンが押された場合
    df_check_list = pd.DataFrame({  # データ確認リストを作成
        ...
        'measuring_instrument_usage': [measuring_instrument_usage],  # 測定機器の使用状況
    })

    # 追加: 選択されたファイル名でtable_qc_status_logから抽出・表示
    try:
        ...
        if file_name:
            query = """
                ...
                ORDER BY Date_Time DESC
            """
            result_df = pd.read_sql(query, view_conn, params=(file_name,))
            st.markdown("#### 選択ファイルの登録済みデータ")
            if not result_df.empty:
                st.dataframe(result_df)
            else:
                st.info("このファイル名で登録されたデータはありません。")
    except Exception as e:
        st.error(f"データ抽出中にエラーが発生しました: {str(e)}")
