...
class QCCheckUI:
    ...
    def render_sidebar() -> Tuple[date, str, str]:
        ...
        # st.date_inputの戻り値を確実にdateオブジェクトに変換
        if isinstance(selected_date, (date, pd.Timestamp)):
            selected_date = selected_date.date() if isinstance(selected_date, pd.Timestamp) else selected_date
        else:
