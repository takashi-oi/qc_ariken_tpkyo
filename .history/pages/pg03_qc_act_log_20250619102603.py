    ...
    with tab1:
        ...
        with st.form("qc_data_form"):
            ...
            implementor = st.selectbox(
                "実施者",
                options=[""] + employee_code_df["Member's_Name"].tolist(),
                key="implementor",
                index=(employee_code_df["Member's_Name"].tolist().index(sidebar_implementor) + 1) if sidebar_implementor in employee_code_df["Member's_Name"].tolist() else 0
            )
