...
class QCCheckApp:
    ...
    async def process_data_batch(self, data: ProcessedData) -> dict:
        ...
        for data_type, df in [
            ...
            try:
                ...
                st.warning(f"{data_type}データの処理中にエラーが発生しました: キー '{e}' が見つかりません")
    ...
    async def process_and_display_westgard_results(
                    ...
                    async with get_db_connection(Config.DATABASE_PATH) as conn:
                        try:
                            ...
                            for type_name, group in (
                                historical_data.groupby("Type")
                            ):
                                st.write(f"#### {type_name} 解析結果")
                                try:
                                    rule_results = check_westgard_rules(group)
                                    # File_Name列を追加
                                    rule_results['File_Name'] = (
                                        data.file_info['File_Name'].iloc[0]
                                    )
                                    st.write("マルチルールチェック結果:")
                                    if not rule_results.empty:
                                        st.dataframe(rule_results)
                                    else:
                                        st.info("マルチルールチェックの結果はありません。")
                                    rule_results.to_sql('table_qc_multi_rule',
                                                        conn,
                                                        if_exists='append',
                                                        index=False)
                                ...
                                IOError,
                                RuntimeError) as e:
                            st.error(f"{data_type}データの処理中にエラーが発生しました: {e}")
        ...
        async with self.error_handler():
            ...
            if uploaded_file and measurer:
                try:
                    ...
                    # データの検証と表示
                    if qc_data and isinstance(qc_data.file_info, pd.DataFrame) and not qc_data.file_info.empty:
                        st.success("✅ ファイルが正常に読み込まれました")
                        
                        # データ概要の表示
                        st.write("### 📊 データ概要")
                        await self.display_data_summary(qc_data)
                        
                        # 詳細データの表示
                        st.write("### 📋 詳細データ")
                        await self.display_detailed_data(qc_data)
                        
                        # データベース操作の処理
                        await self.handle_database_operations(qc_data)
                    else:
                        st.error("❌ データの読み込みに失敗しました。ファイル形式を確認してください。")
                        
                except Exception as e:
                    st.error(f"❌ データ処理中にエラーが発生しました: {str(e)}")
                    st.info("💡 ファイル名の形式が正しいか確認してください（例：CODE_BATCH-YYYYMMDD.xlsx）")
            elif uploaded_file and not measurer:
                st.warning("⚠️ 測定者名を選択してください")
            elif not uploaded_file and measurer:
                st.info("📁 QCデータファイルをアップロードしてください")
            else:
                st.info("📋 QCデータファイルをアップロードし、測定者名を選択してください")
