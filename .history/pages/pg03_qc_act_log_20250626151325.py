            ...
            try:
                ...
                if not multi_rule_df.empty:
                    ...
                    if "Error_type" in multi_rule_df.columns:
                        error_types_raw = (
                            multi_rule_df["Error_type"].dropna().unique().tolist()
                        )
                    else:
                        error_types_raw = []
                    ...
                    for et in error_types_raw:
                        if et != "-":
                            filtered_error_types.append(et)
                    error_types = filtered_error_types
                    error_type_label = "誤差タイプで抽出（'-'以外）"
                    if error_types:
                        selected_error_type = st.selectbox(
                            error_type_label, options=["すべて"] + error_types
                        )
                    else:
                        selected_error_type = "すべて"
                    filtered_multi_rule_df = multi_rule_df.copy()
                    if selected_error_type != "すべて":
                        mask = (
                            filtered_multi_rule_df["Error_type"] == selected_error_type
                        )
                        filtered_multi_rule_df = filtered_multi_rule_df[mask]
                    else:
                        mask = filtered_multi_rule_df["Error_type"] != "-"
                        filtered_multi_rule_df = filtered_multi_rule_df[mask]
