...
class QCCheckApp:
    ...
    async def display_data_summary(self, data: ProcessedData) -> None:
        ...
        metrics = {
            "ファイル情報": (
                len(data.file_info) if
                isinstance(data.file_info, pd.DataFrame)
                else 0
            ),
            "ICデータ": (
                len(data.ic_data) if
                isinstance(data.ic_data, pd.DataFrame)
                else 0
            ),
            "NCデータ": (
                len(data.nc_data) if
                isinstance(data.nc_data, pd.DataFrame)
                else 0
            ),
            "PCデータ": (
                len(data.pc_data)
    ...
    async def check_for_duplicates(self, data: ProcessedData) -> bool:
        ...
        try:
            ...
            for table, check_data in duplicate_checks.items():
                if await self.db_manager.check_duplicate_records(
                    table, check_data):
    ...
    async def handle_database_operations(self, data: ProcessedData) -> None:
        ...
        if st.button("データベース登録／マルチルールチェック開始"):
            if not isinstance(data.file_info, pd.DataFrame) or
            data.file_info.empty:
