...
def create_tables_if_not_exist():
    ...
    create_cursor.execute("""
        CREATE TABLE IF NOT EXISTS qc_mulch_rule (
            ...
            Item_Code TEXT,
            type TEXT,
            PRIMARY KEY (Date_Tome, Batch, Item_Code, type)
        )
    """)
    ...
    create_cursor.execute("""
        CREATE TABLE IF NOT EXISTS qc_multi_rule (
            ...
            Item_Code TEXT,
            QC_RESULTS TEXT,
            PRIMARY KEY (Date_Tome, Batch, Item_Code)
        )
    """)
...
with st.sidebar:
    ...
    # 実施者名の入力
    implementor = st.text_input('実施者名（フルネーム）を入力してください',
                                placeholder='実施者名(フルネーム)をここに入力')
    ...
    cursor.execute("""
        SELECT DISTINCT `Date_Tome`
        FROM table_qc_mulch_rule
        WHERE `Item_Code` = ?
        ORDER BY `Date_Tome` DESC
        LIMIT 5
    """, (Item_Code,))
    result_Date_Tome_list = [Date_Tome[0] for Date_Tome in cursor.fetchall()]

    # 日付リストが存在する場合のみ表示
    if result_Date_Tome_list:
        Date_Tome = st.selectbox('実施日を選択して下さい', result_Date_Tome_list)
    ...
    if Date_Tome:
        ...
        # 選択されたバッチに基づく検査項目の取得
        Batch = st.selectbox('測定「Batch」を選択して下さい',
                             result_time_list)
    ...
    if Batch:
        cursor.execute("""
            ...
            FROM table_qc_mulch_rule
            WHERE `Date_Tome` = ? AND `Batch` = ?
        """, (Date_Tome, Batch))
        # 検査項目のtypeリストの取得
        result_types_list = [type[0] for type in cursor.fetchall()]

        # 検査項目のtypeの選択
        TYPE = st.selectbox('検査項目のtypeを選択して下さい', result_types_list)

    # QC結果の取得を最適化
    if Date_Tome and Batch and TYPE:
        cursor.execute("""
            SELECT DISTINCT `Violated_rule`
            FROM table_qc_mulch_rule
            WHERE `Date_Tome` = ? AND `Batch` = ? AND `Item_Code` = ? AND `type` = ?
        """, (Date_Tome, Batch, Item_Code, TYPE))
        # 検査項目のtypeの結果リストの取得
