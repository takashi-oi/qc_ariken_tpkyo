...
def create_tables_if_not_exist():
    ...
    create_cursor.execute("""
        CREATE TABLE IF NOT EXISTS qc_multi_rule (
            ...
            item_code TEXT,
            type TEXT,
            PRIMARY KEY (date, batch, item_code, type)
        )
    """)
    ...
    create_cursor.execute("""
        CREATE TABLE IF NOT EXISTS qc_multi_rule (
            ...
            item_code TEXT,
            QC_RESULTS TEXT,
            PRIMARY KEY (date, batch, item_code)
        )
    """)
...
with st.sidebar:
    ...
    cursor.execute("""
        ...
        LIMIT 5
    """, (item_code,))
    result_date_list = [date[0] for date in cursor.fetchall()]

    # 日付リストが存在する場合のみ表示
    if result_date_list:
        DATE = st.selectbox('実施日を選択して下さい', result_date_list)
    ...
    # QC結果の取得を最適化
    if DATE and BATCH and TYPE:
        cursor.execute("""
            SELECT DISTINCT `Violated_rule`
            FROM table_qc_mulch_rule
            WHERE `date` = ? AND `batch` = ? AND `item_code` = ? AND `type` = ?
        """, (DATE, BATCH, item_code, TYPE))
        # 検査項目のtypeの結果リストの取得
