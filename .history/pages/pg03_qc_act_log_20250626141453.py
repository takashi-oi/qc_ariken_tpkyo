    ...
    if show_status_log:
        ...
        if selected_file_name:
            try:
                ...
                # ファイル情報の表示
                # st.markdown(f"#### 選択ファイルの情報: {selected_file_name}")

                if not file_info_df.empty:
                    st.subheader("ファイル情報")
                    # デバッグ情報を追加
                    # st.write(f"DataFrameの列数: {len(file_info_df.columns)}")
                    # st.write(f"DataFrameの列名: {list(file_info_df.columns)}")

                    # カラム名を日本語に変更して表示
                    display_file_info = file_info_df.copy()

                    # 列数に応じて列名を設定
                    if len(display_file_info.columns) == 5:
                ...
                if not status_log_df.empty:
                    st.subheader("ステータスログ")
                    st.dataframe(status_log_df)
                else:
