...
def load_qc_data(implementation_date, conn):
    ...
    WHERE `date` <= '{implementation_date}'  # 指定された日付以前のデータのみを取得
    ORDER BY `date` DESC  # 日付の降順でソート
    """
    # SQLクエリを実行してデータフレームに変換
    df_qc_query = pd.read_sql_query(qc_data_query, conn)
    # 日付とバッチで昇順にソートして返す
    return df_qc_query.sort_values(['date', 'Batch'], ascending=True)
...
def main():
    ...
    if year and month:
        ...
        implementation_date = f"{year}-{month.zfill(2)}-{last_day}"
        ...
        with sqlite3.connect('db_folder/qc_data_base.db', timeout=30) as conn:
            ...
            # QCデータを読み込み、基本統計量を計算
            df_qc_data = load_qc_data(implementation_date, conn)
