...
class MasterDataLoader:
    ...
    def load_employee_data(cls, force_reload: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            if not file_path.exists():
                logger.error(
                    "従業員マスターファイルが存在しません: %s", file_path)
                return pd.DataFrame()

            cls._employee_data_cache = pd.read_excel(file_path)
            logger.info(
                "従業員マスター読み込み成功: %d件", len(cls._employee_data_cache))
            return cls._employee_data_cache
        except (FileNotFoundError,
                pd.errors.EmptyDataError,
                pd.errors.ParserError) as e:
            logger.error("従業員マスターの読み込みエラー: %s", e)
            return pd.DataFrame()
...
def main():
    ...
    with tab1:
        ...
        with st.form("qc_data_form"):
            ...
            # 実施者選択
            employee_options = []
            if not employee_df.empty and "社員名" in employee_df.columns:
                employee_options = employee_df["社員名"].tolist()
            implementor = st.selectbox("実施者", options=[""] + MasterDataLoader.load_employee_data()["社員名"].tolist())
