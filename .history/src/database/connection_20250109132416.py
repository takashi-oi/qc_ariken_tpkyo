    ...
    try:
        ...
        async with get_db_connection(config.DATABASE_PATH) as conn:
            ...
            try:
                ...
                # ファイル情報のバリデーション
                if file_info is None or file_info.empty:
                    raise ValueError("ファイル情報が無効です")

                # 必須カラムの存在確認
                required_columns = ['Date_Time', 'Batch', 'Item_Code']
                if not all(col in file_info.columns for col in required_columns):
                    raise ValueError("ファイル情報に必須カラムが不足しています")
