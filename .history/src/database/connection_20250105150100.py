...
async def check_duplicate_records(conn, table_name: str, file_info: pd.DataFrame) -> bool:
    """レコードの重複をチェック"""
    try:
        cursor = conn.cursor()
        # file_infoから日付、バッチ、アイテムコードを取得
        date = file_info['Date_Tome'].iloc[0]
        batch = file_info['Batch'].iloc[0]
        item_code = file_info['Item_Code'].iloc[0]

        query = f"""
        SELECT COUNT(*) FROM {table_name}
        WHERE Date_Tome = ? AND Batch = ? AND Item_Code = ?
        """
