...
def _validate_data_quality(file_info: pd.DataFrame,
    ...
    try:
        ...
        # ICデータの検証
        if not ic_data.empty and not all(col in ic_data.columns for col in qc_required_columns):
            logger.error("ICデータに必須カラムが不足しています")
            return False

        # NCデータの検証
        if not nc_data.empty and not all(col in nc_data.columns for col in qc_required_columns):
