...
def _validate_data_quality(file_info: pd.DataFrame,
    ...
    try:
        ...
        # PCデータの検証
        if not pc_data.empty and not all(
            col in pc_data.columns for col in qc_required_columns
        ):
        ...
        logger.error("空のデータフレームエラー: %s", e)
        return False
    except (ValueError, TypeError) as e:
        logger.error("データ型エラー: %s", e)
        return False
    except KeyError as e:
        logger.error("キーエラー: %s", e)
        return False
    ...
    try:
        ...
        for col, dtype in required_columns.items():
            if col not in file_info.columns:
                logger.error("必須カラム %s が見つかりません", col)
                return False

            if not all(isinstance(x, dtype) for x in file_info[col].dropna()):
                logger.error("カラム %s のデータ型が不正です", col)
                return False

        # データの整合性チェック
        if not file_info['Date_Time'].map(_is_valid_date).all():
            logger.error("不正な日付形式が含まれています")
            return False

        return True
[{
	"resource": "/Users/user/Documents/GitHub/qc_ariken_tpkyo/src/database/connection.py",
