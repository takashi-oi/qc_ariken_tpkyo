...
class MultiRule:
    ...
    def evaluate_rules(self, sd_conversions):
        ...
        Returns:
            list: 各ルールの評価結果（[1-2s, 1-3s, 2-2s, R-4s, 4-1s, 8x]の順）
        """
        ...
        # 各ルールの評価結果を格納するリスト
        rule_results = [0, 0, 0, 0, 0, 0]  # [1-2s, 1-3s, 2-2s, R-4s, 4-1s, 8x]
        rule_found = False

        # 1-2s
        latest_sd = abs(sd_conversions[latest_idx])
        if 2 <= latest_sd <= 3 and not rule_found:
            rule_results[0] = 1
            rule_found = True
        # 1-3s
        if latest_sd > 3 and not rule_found:
            rule_results[1] = 1
            rule_found = True
        # 2-2s
        if len(sd_conversions) >= 2 and not rule_found:
        ...
        if len(sd_conversions) >= 2 and not rule_found:
            last_two = sd_conversions[-2:]
            if abs(last_two[0] - last_two[1]) > 4:
                rule_results[3] = 1
                rule_found = True
        # 4-1s
        if len(sd_conversions) >= 4 and not rule_found:
            last_four = sd_conversions[-4:]
            if all(sd < -1 for sd in last_four) or all(sd > 1 for sd in last_four):
                rule_results[4] = 1
                rule_found = True
        # 8x
        if len(sd_conversions) >= 8 and not rule_found:
            last_eight = sd_conversions[-8:]
            if all(sd > 0 for sd in last_eight) or all(sd < 0 for sd in last_eight):
                rule_results[5] = 1
                rule_found = True
        return rule_results
    ...
    def apply_rules(
        ...
        for value, sd_conv, qc_type, item_code, dye, b, m, d, meas, l in zip(
            ...
            # Westgardルールの評価
            westgard_rule_list = self.evaluate_rules(sd_conv)
            rule_names = ["1-2s", "1-3s", "2-2s", "R-4s", "4-1s", "8x"]
            westgard_rules = dict(zip(rule_names, westgard_rule_list))
            ...
            if any(westgard_rule_list) or error_type:
                result["Judgment"] = "Fail"
                # 違反したルールを抽出
                error_rules = [
...
def check_westgard_rules(
    ...
    if historical_data is not None:
        ...
        combined_data = pd.concat(
            [historical_data, data], ignore_index=True
    ...
    processed_data = {
        ...
        "sd_conversions": combined_data.get("SD_Conversion", pd.Series()).tolist(),
