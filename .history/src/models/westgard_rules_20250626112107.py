"""Westgardマルチルールのモデル"""  # Westgardマルチルールを適用するためのモデルを定義します。
...
class MultiRule:
    ...
    def __init__(self,
                 type_conditions: Dict[str,
                                       List[Dict[str,
                                                 Union[float, str]]]],
                 db_connection):  # データベース接続を受け取ります。
        # タイプごとの条件を初期化
        self.type_conditions = type_conditions  # タイプごとの条件をインスタンス変数に保存します。
        self.db_connection = db_connection  # データベース接続をインスタンス変数に保存します。
    ...
    def check_duplicates(self, data: pd.DataFrame, table_name: str) -> bool:
        ...
        for _, row in data.iterrows():  # データフレームの各行をループ処理します。
            ...
            cursor.execute(query, (row['item_code'],  # SQLクエリを実行し、行のデータを渡します。
                                   row['batch'],
                                   row['date'],
                                   row['model']))
    ...
    def import_to_database(self, file_info: pd.DataFrame,
        ...
        try:
            ...
            for table_name, data in tables.items():  # 各テーブルに対してループ処理します。
            ...
            for table_name, data in tables.items():  # 各テーブルに対してループ処理します。
                if not data.empty:  # データが空でない場合
                    columns = ', '.join(data.columns)  # カラム名をカンマ区切りで取得します。
                    # プレースホルダーを作成します。
            ...
            print("空のデータフレームが検出されました")  # エラーメッセージを表示します。
    ...
    def apply_type_specific_rules(
        ...
        # タイプに対応する条件が存在するか確認
        if qc_type not in self.type_conditions:  # 指定されたQCタイプが条件に存在しない場合
            return error, error_message  # エラーなしで返します。

        # タイプごとの条件をチェック
        conditions = self.type_conditions[qc_type]  # QCタイプに対応する条件を取得します。
    ...
    def evaluate_rules(self, sd_conversions):
        ...
        if len(sd_conversions) >= 2:
            ...
            results['2-2s'] = 1 if all(abs(sd) >= 2 for sd in last_two) else 0

        # 直近4点でのR-4sルールの評価
        if len(sd_conversions) >= 4:
            last_four = sd_conversions[-4:]
            results['R-4s'] = 1 if max(last_four) - min(last_four) >= 4 else 0

        # 直近4点での4-1sルールの評価
        if len(sd_conversions) >= 4:
            last_four = sd_conversions[-4:]
            results['4-1s'] = 1 if all(abs(sd) >= 1 for sd in last_four) else 0
        ...
        if len(sd_conversions) >= 8:
            ...
            results['8x'] = 1 if (
                ...
                all(sd < 0 for sd in last_eight)) else 0
    ...
    def apply_rules(
        ...
        dyes = dyes or ['-'] * len(qc_data)

        for value, sd_conv, qc_type, item_code, dye, b, m, d, meas, l in zip(
                qc_data, sd_conversions, types, item_codes, dyes, batch,
                model, date, measurer_list, lot):
            result = {
                'Item_Code': item_code,
                'Batch': b,
                'Model': m,
            ...
            # Westgardルールの評価
            westgard_rules = self.evaluate_rules(sd_conv)
            ...
            if any(westgard_rules.values()) or error_type:
                ...
                elif (westgard_rules['2-2s'] or westgard_rules['4-1s'] or
                      westgard_rules['8x']):
        ...
        # 指定のカラム順に並べ替え
        column_order = [
            'Measurer', 'Date_Time', 'Item_Code', 'Batch', 'Model', 'Judgment',
            'Error_type', 'Type_error', 'Violated_rule', 'Type', 'Dye', 'Ct',
            'SD_Conversion', 'Lot_Number', '1-2s', '1-3s', '2-2s', 'R-4s', '4-1s', '8x', 'File_Name'
        ]
...
def check_westgard_rules(
    ...
    # MultiRuleインスタンスを作成
    westgard_checker = MultiRule(type_conditions={
    ...
    # データを処理用に整形
    processed_data = {
        'qc_data': combined_data.get('Ct', pd.Series()).tolist(),
        'sd_conversions': combined_data.get(
            'SD_Conversion', pd.Series()).tolist(),
        'types': combined_data.get('Type', pd.Series()).tolist(),
        'item_codes': combined_data.get('Item_Code', pd.Series()).tolist(),
        'batch': combined_data.get('Batch', pd.Series()).tolist(),
        'model': combined_data.get('Model', pd.Series()).tolist(),
        'date': combined_data.get('Date_Time', pd.Series()).tolist(),
        'measurer': combined_data.get('Measurer', pd.Series()).tolist(),
        'lot_number': combined_data.get('Lot_Number', pd.Series()).tolist(),
        'dyes': ['-'] * len(combined_data)
    }

    # Westgardルールを適用
    results_df = westgard_checker.apply_rules(**processed_data)

    # 最新の日時のレコードのみをフィルタリング
    if not data.empty and 'Date_Time' in results_df.columns:
        try:
            latest_date = data['Date_Time'].max()
            results_df = results_df[results_df['Date_Time'] == latest_date]
        except (KeyError, ValueError):
            # フィルタリングに失敗した場合は、元の結果をそのまま返す
            pass
