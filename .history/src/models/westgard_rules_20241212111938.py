...
class WestgardMultiRule:
    ...
    def apply_rules(self,
        ...
        for i, sd in enumerate(sd_values):
            ...
            if abs(sd - mean) > 4 * std:
                results.loc[i, 'judgment'] = 'NG'
                results.loc[i, 'Error_type'] = '4-1s違反'
                results.loc[i, 'Violated_rule'] = '4-1s'

            # 8xルールの適用
            if abs(sd - mean) > 8 * std:
                results.loc[i, 'judgment'] = 'NG'
                results.loc[i, 'Error_type'] = '8x違反'
                results.loc[i, 'Violated_rule'] = '8x'

        # 追加のルールを必要に応じて実装
        results['1-2s'] = np.where(
            abs(results['SD_conversion'] - mean) > 2 * std, 1, 0)
        results['1-3s'] = np.where(
            abs(results['SD_conversion'] - mean) > 3 * std, 1, 0)
        results['2-2s'] = np.where(
            abs(results['SD_conversion'] - mean) > 2 * std, 1, 0)
        results['R-4s'] = np.where(
            abs(results['SD_conversion'] - mean) > 4 * std, 1, 0)
        results['4-1s'] = np.where(
            abs(results['SD_conversion'] - mean) > 4 * std, 1, 0)
        results['8x'] = np.where(
            abs(results['SD_conversion'] - mean) > 8 * std, 1, 0)
