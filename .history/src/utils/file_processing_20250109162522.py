...
class QCDataProcessor:
    ...
    async def _process_data(self, uploaded_df: DataFrame, file_info: DataFrame) -> ProcessedData:
        ...
        try:
            processed_df = await self._preprocess_data(uploaded_df, file_info)
            ...
            ic_data = organize_data(ic_data, columns) if not ic_data.empty else pd.DataFrame(columns=columns)
            nc_data = organize_data(nc_data, columns) if not nc_data.empty else pd.DataFrame(columns=columns)
            pc_data = organize_data(pc_data, pc_columns) if not pc_data.empty else pd.DataFrame(columns=pc_columns)
