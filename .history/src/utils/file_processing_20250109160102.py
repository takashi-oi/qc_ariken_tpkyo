...
class QCDataProcessor:
    ...
    async def _process_data(self,
        ...
        try:
            # データを前処理
            processed_df = self._preprocess_data(uploaded_df)  # 修正: 位置引数を1つに変更
            # QCコントロールファイルのパス
            qc_control_path = 'master_folder/qc_control.xlsx'
            # QCコントロールデータを読み込む
            qc_control_df = pd.read_excel(qc_control_path, engine='openpyxl')
            # データをマージ
            processed_df = pd.merge(processed_df, qc_control_df, how='inner')
            return processed_df  # 処理されたデータを返す
        except ValueError as e:
            # エラーを発生
            raise ValueError(f"Error processing QC control data: {str(e)}") from e
    ...
    def _preprocess_data(self,
        ...
        try:
            ...
            # カラム名の確認
            required_columns = {'Sample Type', 'Ct', 'Type'}
            available_columns = set(processed_df.columns)
            ...
            if missing_columns:
                ...
                for missing_col in missing_columns:
                    if missing_col.lower() in case_insensitive_columns:
                        # カラム名を正しい形式に変更
                        processed_df = processed_df.rename(
                            columns={
                                case_insensitive_columns[missing_col.lower()]:
                                missing_col
                            }
                        )
                    else:
                        raise ValueError(f"必要なカラムが見つかりません: {missing_col}")

            return processed_df

        except Exception as e:
            raise ValueError(f"データの前処理中にエラーが発生: {str(e)}") from e
