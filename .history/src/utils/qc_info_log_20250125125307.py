...
def load_item_codes():
    ...
    try:
        ...
        column_pairs = [
            ...
            ('Measurement Item', ['Measurement Item', 'Measurement_Item', 'MeasurementItem', '測定項目', '項目名'])
        ]
        column_mapping = {target: next((col for col in possibilities if col in columns), None) for target, possibilities in column_pairs}
...
with st.sidebar:
    ...
    # 検査項目の選択と保存
    item_code = st.selectbox('検査項目を選択してください', ITEM_CODE_SELECT['Item_Code'], format_func=lambda x: ITEM_MAPPING.get(x, 'Unknown'))
