...
with st.sidebar:
    ...
    try:
        ...
        st.stop()  # エラーメッセージを表示した後、処理を停止します
    except (pd.errors.EmptyDataError, pd.errors.ParserError, PermissionError) as e:
        st.error(f"ファイルエラー: {str(e)}")  # エラー内容を表示します
        st.stop()  # エラーメッセージを表示した後、処理を停止します
    item_code = st.selectbox('検査項目を選択してください', ITEM_CODE_SELECT['Item_Code'],
                             format_func=lambda x: x if x in ITEM_CODE_SELECT['Item_Code'] else 'Unknown') 'Item_Code'] else 'Unknown')
