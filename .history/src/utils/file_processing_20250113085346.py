...
class QCDataProcessor:
    ...
    @staticmethod
    def _organize_data(df: DataFrame, cols: list) -> DataFrame:
        """データフレームの列を指定された順序で整理する

        Args:
            df (DataFrame): 整理対象のデータフレーム
            cols (list): 希望する列の順序リスト

        Returns:
            DataFrame: 列が整理されたデータフレーム
        """
        if df.empty:
    ...
    async def _process_data(self,
        ...
        try:
            ...
            # 列の整理
            ic_data = self._organize_data(ic_data, columns)
            nc_data = self._organize_data(nc_data, columns)
            pc_data = self._organize_data(pc_data, pc_columns)
...
def save_to_database(file_info, ic_data, nc_data, pc_data):
    """QCデータをデータベースに保存する関数

    Args:
        file_info (DataFrame): ファイル情報のデータフレーム
        ic_data (DataFrame): ICデータのデータフレーム
        nc_data (DataFrame): NCデータのデータフレーム
        pc_data (DataFrame): PCデータのデータフレーム
    ...
    try:
        # file_infoテーブルにデータを挿入
        cursor.execute("""
            INSERT INTO file_info (timestamp, filename, path)
            VALUES (?, ?, ?)
        """, (current_time, file_info['filename'], file_info['path']))

        inserted_id = cursor.lastrowid  # 最後に挿入された行のID

        # ic_dataテーブルにデータを挿入
        for data in ic_data:
            cursor.execute("""
                INSERT INTO ic_data (file_id, value1, value2)
                VALUES (?, ?, ?)
            """, (inserted_id, data['value1'], data['value2']))

        # 同様にnc_dataとpc_dataも挿入
        for data in nc_data:
            cursor.execute("""
                INSERT INTO nc_data (file_id, value1, value2)
                VALUES (?, ?, ?)
            """, (inserted_id, data['value1'], data['value2']))

        for data in pc_data:
            cursor.execute("""
                INSERT INTO pc_data (file_id, value1, value2)
                VALUES (?, ?, ?)
            """, (inserted_id, data['value1'], data['value2']))
