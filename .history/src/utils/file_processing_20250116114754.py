...
class ProcessedData:
    ...
    def __post_init__(self):
        """データクラスの初期化後の検証"""
        if not isinstance(self.file_info, pd.DataFrame):
            raise ValueError("file_infoはDataFrameである必要があります")
        if not isinstance(self.ic_data, pd.DataFrame):
    ...
    def is_valid(self) -> bool:
        ...
        try:
            ...
            if not all(isinstance(df, pd.DataFrame) and not df.empty for df in required_dfs):
                return False
