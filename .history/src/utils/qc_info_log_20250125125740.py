...
with col2:
    ...
    measuring_instrument_usage = st.radio('測定機器の使用状況を選択してください。異常等がある場合は機器保守参照を選択してください。',
...
def load_item_codes():
    ...
    try:
        ...
        column_mapping = {
            'Item_Code': next((col for col in item_code_select.columns if col in ['Item_Code', 'ItemCode', '項目コード']), None),
            'Measurement Item': next((col for col in item_code_select.columns if col in ['Measurement Item', 'Measurement_Item', 'MeasurementItem', '測定項目', '項目名']), None)
        }

        for target, col in column_mapping.items():
            if col is None:
                st.error(f"必要な列 '{target}' が見つかりません。利用可能な列: {', '.join(item_code_select.columns)}")
                st.stop()
