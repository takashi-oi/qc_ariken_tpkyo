...
class QCDataApp:
    ...
    async def run(self):
        ...
        try:
            ...
            with st.sidebar:
                # 測定者名の入力
                measurer = st.text_input("測定者名", label_visibility="visible")
            ...
            if uploaded_file and measurer:
                await self.processor.process_and_display(uploaded_file, measurer)
                
        except Exception as e:
            st.error(f"Application error: {str(e)}")
            raise RuntimeError(f"QC Data processing failed: {str(e)}")
    ...
    def _display_ic_data(ic_data: DataFrame):
        ...
        try:
            ...
            if 'verdict' in ic_data.columns:
                ...
                cols[2].metric("Pass Rate", f"{(pass_count / total * 100):.1f}%" if total > 0 else "0%")

        except Exception as e:
            raise ValueError(f"Error displaying IC data: {str(e)}")
    ...
    def _display_nc_data(nc_data: DataFrame):
        ...
        try:
            ...
            if 'Ct' in nc_data.columns and not nc_data['Ct'].empty:
                ct_values = pd.to_numeric(nc_data['Ct'].replace(['-', None], pd.NA), errors='coerce')
...
class QCDataProcessor:
        ...
        try:
            ...
            if not pc_data.empty:
                pc_data['verdict'] = pc_data['Ct'].apply(
                    lambda x: 'Pass' if pd.to_numeric(x, errors='coerce') > 0 else 'Fail'
                )
    ...
    def _preprocess_data(self, df: DataFrame, file_info: DataFrame) -> DataFrame:
        ...
        try:
            ...
            processed_df['Sample Type'] = processed_df['Sample Type'].map(type_mapping)

            # 無効なサンプルタイプの確認
            invalid_types = set(processed_df['Sample Type'].dropna()) - set(type_mapping.values())
            if invalid_types:
                raise ValueError(f"Invalid sample types found: {invalid_types}")
