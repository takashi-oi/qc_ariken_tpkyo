    ...
    def _get_file_info(self, file: Any, measurer: str) -> DataFrame:
        ...
        try:
            ...
            return pd.DataFrame({
                'File_Name': [file.name],
                'Measurer': [measurer],
                'Item_Code': [code_batch[0]],
                'Batch': [code_batch[1]],
                'Model': ['default_model'],
                'Date_Time': [pd.to_datetime(name_parts[1]).strftime('%Y/%m/%d %H:%M:%S')]
            })
        except (IndexError, ValueError) as e:
            raise ValueError(f"Invalid file name format: {file.name}") from e

    def _process_data(self, uploaded_df: DataFrame, file_info: DataFrame) -> ProcessedData:
        """アップロードされたデータを処理"""
        try:
            ...
            return ProcessedData(
                ...
                nc_data=processed_df[(processed_df['Sample Type'
                                                   ] == 'Negative') &
                                    (processed_df['Type'] != 'IC')].copy(),
                pc_data=processed_df[(processed_df['Sample Type'
                                                   ] == 'Positive') &
                                    (processed_df['Type'] != 'IC')].copy()
            )
        except Exception as e:
