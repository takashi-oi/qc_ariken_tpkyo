...
class QCDataProcessor:
    ...
    async def _get_file_info(self, file: Any, measurer: str) -> DataFrame:
        ...
        try:
            file_info = pd.DataFrame({
                ...
                'model': [file.name.split('_')[1].split('-')[0]],  # ファイル名から_と-の間の文字を抽出
    ...
    async def _process_data(self,
        ...
        try:
            ...
            if ic_data.empty:
                ic_data = pd.DataFrame(columns=['Sample Type', 'Ct', 'verdict'])
            else:
            ...
            if nc_data.empty:
                nc_data = pd.DataFrame(columns=['Sample Type', 'Ct', 'verdict'])
            else:
            ...
            if pc_data.empty:
                pc_data = pd.DataFrame(columns=['Sample Type', 'Ct', 'verdict'])
            else:
