...
from dataclasses import dataclass  # データクラスを作成するためのライブラリ
import pandas as pd  # データ操作のためのライブラリ
import streamlit as st  # Webアプリケーション作成のためのライブラリ
from pandas import DataFrame  # データフレームを扱うためのクラス
from typing import Any  # 任意の型を指定するためのライブラリ
...
class QCDataApp:
    ...
    def __init__(self):
        ...
        self.processor = QCDataProcessor()  # データ処理用のインスタンスを作成

    async def run(self):
        """アプリケーションを実行"""
        try:
            st.set_page_config(page_title="QC Data Analysis", layout="wide")  # ページ設定
            st.title("QC Data Analysis")  # タイトルを設定

            with st.sidebar:  # サイドバーの設定
                employee_data = pd.read_excel("master_folder/employee_code.xlsx")  # 従業員データを読み込む
                Measurer = st.selectbox("測定者名", employee_data["Member's_Name"].tolist())  # 測定者名を選択

                uploaded_file = st.file_uploader("測定結果Excelファイルをアップロードして下さい", type=['xlsx', 'xls'])  # ファイルアップロード

            if uploaded_file and Measurer:  # アップロードされたファイルと測定者名がある場合
                await self.processor.process_and_display(uploaded_file, Measurer)  # データ処理と表示を実行

        except Exception as e:
            st.error(f"Application error: {str(e)}")  # エラーメッセージを表示
            raise RuntimeError(f"QC Data processing failed: {str(e)}") from e  # エラーを再発生
...
class QCDataDisplay:
    ...
    def display_all_data(processed_data: ProcessedData):
        ...
        st.write("## QC Data Analysis Results")  # 結果のタイトルを表示

        if not processed_data.is_valid():  # データが無効な場合
