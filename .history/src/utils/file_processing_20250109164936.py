...
from database.connection import get_db_connection, check_duplicate_records, import_to_database
        ...
        try:
            ...
            if uploaded_file and measurer:
                ...
                if st.button("データベースにインポート"):
                    async with get_db_connection('db_folder/qc_data_base.db') as conn:
                        # 重複チェック
                        is_duplicate = await check_duplicate_records(
                            conn,
                            'table_qc_test_data_file',
                            processed_data.file_info
                        )
                        
                        if is_duplicate:
                            st.warning("このデータは既にデータベースに存在します")
                        else:
                            # データベースにインポート
