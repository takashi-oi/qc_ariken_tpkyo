...
class QCDataProcessor:
    ...
    async def _process_data(self,
        ...
        try:
            # データを前処理
            processed_df = self._preprocess_data(uploaded_df,
            file_info)
            # QCコントロールファイルのパス
            ...
            # 判定を適用する関数
            def apply_verdict(data: DataFrame, condition: str) -> DataFrame:
                if data.empty:
                    return pd.DataFrame(columns=['Sample Type', 'Ct', 'verdict'])
                else:
                    data['verdict'] = data['Ct'].apply(
                        lambda x: 'Pass' if condition(x) else 'Fail'
                    )
                    return data

            # ICデータの判定
            ic_data = apply_verdict(ic_data, lambda x: pd.to_numeric(x, errors='coerce') >= 10)

            # NCデータの判定
            nc_data = apply_verdict(nc_data, lambda x: pd.isna(x) or x == '-')

            # PCデータの変換
            if pc_data.empty:
                pc_data = pd.DataFrame(columns=['Sample Type', 'Ct', 'SD_Conversion'])
            else:
            ...
            # 各データフレームの列を整理
            def organize_data(data: DataFrame, columns: list) -> DataFrame:
                if not data.empty:
                    data = data[columns]
                    data['Ct'] = pd.to_numeric(data['Ct'], errors='coerce')
                return data

            ic_data = organize_data(ic_data, ['verdict', 'Item_Code', 'Date_Time', 'Batch', 'Model', 'Measurer', 'Dye', 'Type', 'Lot Number', 'Sample Type', 'Ct', 'CL', 'SD'])
            nc_data = organize_data(nc_data, ['verdict', 'Item_Code', 'Date_Time', 'Batch', 'Model', 'Measurer', 'Dye', 'Type', 'Lot Number', 'Sample Type', 'Ct', 'CL', 'SD'])
            pc_data = organize_data(pc_data, ['SD_Conversion', 'Item_Code', 'Date_Time', 'Batch', 'Model', 'Measurer', 'Dye', 'Type', 'Lot Number', 'Sample Type', 'Ct', 'CL', 'SD'])
