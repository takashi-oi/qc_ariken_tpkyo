...
class DatabaseManager:
    ...
    def fetch_qc_data(self):
        ...
        try:
            ...
            WHERE Date_Time >= Date_Time('now', '-30 days')
    ...
    def save_check_log(self, check_log_df):
        ...
        try:
            ...
            # データ型の変換と検証
            check_log_df['Date_Time'] = pd.to_Date_Timetime(check_log_df['Date_Time']).dt.Date_Time
...
class QCCheckUI:
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            # 日付選択（未来日付を制限）
            Date_Time = st.Date_Time_input(
                "日付を選択してください",
                value=pd.to_Date_Timetime('today').Date_Time(),
                max_value=pd.to_Date_Timetime('today').Date_Time(),
                key="Date_Time_input"
            )
            ...
            return Date_Time, identifier, Measurer

    # チャートのレンダリング
    def render_charts(self, Date_Time, identifier, Measurer):
        """チャートのレンダリング"""
...
def main():
    ...
    try:
        ...
        # サイドバーのレンダリング
        Date_Time, identifier, Measurer = ui.render_sidebar()
        ...
        check_logs = ui.render_charts(Date_Time, identifier, Measurer)
