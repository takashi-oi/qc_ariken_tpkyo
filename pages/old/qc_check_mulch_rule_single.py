...
from src.database.connection import import_to_database
from src.models.westgard_rules import MultiRule
from src.utils.old.data_conversion import check_duplicate_entry
from src.utils.file_processing import get_file_info, process_control_data
            ...
            with st.spinner("分析を実行中..."):
                for file in uploaded_files:
                    try:
                        ...
                        # マルチルール分析の実行
                        westgard_multi_rule = WestgardMultiRule({})
                        for type_name, group in df_qc_data.groupby('Type'):
                            ...
                            if 'sd_conversion' in results.columns:
                                st.line_chart(
                                    results.set_index('date')['sd_conversion'],
