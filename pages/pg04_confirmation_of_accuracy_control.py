...
def get_database_manager():
    ...
    manager = DatabaseManager(AppConfig.DATABASE_PATH)

    # テーブル作成とカラム追加（初回のみ実行）
    try:
        asyncio.run(manager.create_tables())
        _ensure_columns(AppConfig.DATABASE_PATH)
...
def _ensure_columns(db_path: str):
    ...
    try:
        ...
        logger.error(f"カラム追加処理エラー: {e}")


class QCCheckRepository:
    """QCチェックデータへのアクセスを管理するクラス"""
            ...
            with duckdb.connect(self.db_path) as conn:
                ...
                if not qc_data.empty and "date_time" in qc_data.columns:
                    ...
                    if "Batch" in qc_data.columns:
                        ...
                        # Batchカラムがない場合は日付のみ
                        qc_data["date_time_batch"] = qc_data[
                    ...
                    if "sd_conversion" in qc_data.columns:
                        qc_data["sd_conversion"] = qc_data[
                            "sd_conversion"].round(3)
...
class QCCheckUI:
    ...
    def render_sidebar(self) -> tuple[str, str, str, str]:
        ...
        with st.sidebar:
            ...
            # データ取得ボタンを追加
            if st.button("データ取得"):
                st.session_state["check_data"] = self.repository.fetch_data(
...
def main() -> None:
    ...
    try:
        # データベースマネージャーの初期化（キャッシュ利用）
        get_database_manager()
        # データアクセスクラスの初期化
        repository = QCCheckRepository(AppConfig.DATABASE_PATH)
        ui = QCCheckUI(repository)
        ...
        if st.button("チェック結果を保存"):
            if ui.repository.save_qc_check(
