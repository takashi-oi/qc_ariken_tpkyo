...
class QCDataAccess:
    ...
    def _create_indices(self) -> None:
        ...
        try:
            # get_db_connection は async context manager なので同期コードで使うには工夫が必要だが、
            # ここでは単純に connect を使うか、asyncio.run(self._create_indices_async()) とする
            # 簡便のため duckdb.connect を直接使用 (connection pool 的なものは duckdb にはないが)
            # 実際には DatabaseManager の create_tables で定義すべきだが、互換性のため残す
            with duckdb.connect(self.db_path) as conn:
                ...
                # conn.commit() # DuckDB in auto-commit mode usually
    ...
    def get_file_info_records(self, file_name: str) -> pd.DataFrame:
        ...
        try:
            ...
            """
            with duckdb.connect(self.db_path) as conn:
                return pd.read_sql_query(query, conn, params=(file_name,))
        except duckdb.Error as e:
        ...
        try:
            ...
            with duckdb.connect(self.db_path) as conn:
                status_log_df = pd.read_sql_query(
                    query_status, conn, params=(file_name,)
                )

                # table_qc_file_infoからデータ抽出
                # (Re-use conn if simplified, but calling existing method)
                # Note: get_file_info_records opens its own connection now.
                ...
                multi_rule_df = pd.read_sql_query(
                    query_multi_rule, conn, params=(file_name,)
                )
...
def main():
    ...
    employee_df = load_employee_data_cached()
    ...
    def get_file_name_options() -> List[str]:
        """ファイル名リストを取得（キャッシュ付き）"""
        try:
            query = (
            ...
            with duckdb.connect(Config.db_path) as _conn:
    ...
    file_name_options = get_file_name_options()
    ...
    # データベース初期化（接続は既に取得済み）
    try:
        data_access = QCDataAccess(Config.db_path)
