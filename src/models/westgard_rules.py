...
class MultiRule:
    ...
    def check_duplicates(self, data: pd.DataFrame, table_name: str) -> bool:
        ...
        for _, row in data.iterrows():
            ...
            cursor.execute(query, (row['item_code'], row['batch'], 
                                 row['date'], row['model']))
    ...
    def import_to_database(self, file_info: pd.DataFrame,
        ...
        try:
            ...
            for table_name, data in tables.items():
                if self.check_duplicates(data, table_name):
                    return False
...
def check_westgard_rules(data: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    Returns:
        pd.DataFrame: チェック結果を含むデータフレーム
    """
    results = []
    
    # データを項目コードとバッチでグループ化
    grouped = data.groupby(['item_code', 'batch'])
    
    for (item_code, batch), group in grouped:
        # 測定値の統計量を計算
        ...
        for _, row in group.iterrows():
            ...
            # 1-2s ルール: 管理限界（±2SD）を超える
            if abs(value - mean) > 2 * std:
                violations.append('1-2s')

            # 1-3s ルール: 警戒限界（±3SD）を超える
            ...
            result = {
                ...
                'status': 'NG' if violations else 'OK'
            }
            results.append(result)
